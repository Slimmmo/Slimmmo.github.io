...
advApp.controller('advController', ['$document', '$filter', '$scope', function($document, $filter, $scope) {
  ...
  function applyTuple(loc, row) {
    ...
    for (; i < row.length; i++) {
      if (typeof row[i] === 'object') {
        ...
        if (applyRow < 10) {
          if (applyType === 0) {
            loc.investments[applyRow][2] *= row[i][1];
          } else {
            if (row[i][1] !== 2) {
  ...
  function calcRecommendations(loc) {
    ...
    for (; i < loc.investments.length; i++) {
      ...
      for (j = 0; j < inc.length; j++) {
        ...
        if ((tempPlanet.totalMoneyPerSecond - loc.totalMoneyPerSecond) / (tempUnlock / loc.totalMoneyPerSecond) > max) {
    ...
    for (i = 0; i < 11; i++) {
      ...
      calcState(tempPlanet);
      if ((tempPlanet.totalMoneyPerSecond - loc.totalMoneyPerSecond) / (loc.cashUpgrades[j][0] / loc.totalMoneyPerSecond) > max) {
  ...
  function calcState(loc) {
    ...
    for (i = 0; i < loc.investments.length; i++) {
      ...
      loc.investments[i][4] = loc.investments[i][2] / loc.investments[i][3]
      loc.totalMoneyPerSecond += loc.investments[i][4];
    }
    for (i = 0; i < loc.investments.length; i++) {
      loc.investments[i][5] = loc.investments[i][4] * 100 / loc.totalMoneyPerSecond;
    }
  ...
  function loadDefaults() {
    ...
    $scope.moon.investments = [
      ...
      ['Payday Clone', 0, 0, 0, 0, 0],
      ['Moon Express', 0, 0, 0, 0, 0],
      ['Oxygen Bar', 0, 0, 0, 0, 0],
      ['Helium-3 Farm', 0, 0, 0, 0, 0],
      ['Cheese Mine', 0, 0, 0, 0, 0],
      ['Amusement Park', 0, 0, 0, 0, 0],
      ['Werewolf Colony', 0, 0, 0, 0, 0],
